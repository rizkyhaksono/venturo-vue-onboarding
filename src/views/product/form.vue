      ...
      <BRow>
        <BCol lg="6" md="12">
          ...
          <div v-else class="mb-4">
            <ImageCropper :aspectRatio="1 / 1" :uploadText="'Letakkan gambar disini atau klik untuk mengunggah'"
          ...
          <BForm class="form-horizontal" role="form">
            <BRow class="mb-3">
              ...
              <BCol md="10">
                <input class="form-control" :class="{ 'is-invalid': !!(errorList && errorList.name) }" id="form-name"
                  placeholder="Masukkan Nama Product" v-model="formModel.name" />
            ...
            <BRow class="mb-3">
              ...
              <BCol md="10">
                <BFormSelect class="form-select" v-model="formModel.product_category_id"
                  :class="{ 'is-invalid': !!(errorList && errorList.product_category_id) }" id="form-category"
                  :style="{ color: formModel.product_category_id === '' ? '#7c8094' : '' }">
                  <BFormSelectOption :value="''" :disabled="true">Pilih Kategori</BFormSelectOption>
                  <BFormSelectOption @change="updateCategory(cat)" v-for="cat in category" :key="cat" :value="cat.id">
                    {{ cat.name }}
                  </BFormSelectOption>
                </BFormSelect>
                <template v-if="!!(errorList && errorList.product_category_id)">
                  <div class="invalid-feedback" v-for="(err, index) in errorList.product_category_id" :key="index">
                    <span>{{ err }}</span>
                  </div>
                </template>
              </BCol>
            ...
            <BRow class="mb-3">
              ...
              <BCol md="10">
                <input type="number" class="form-control" :class="{ 'is-invalid': !!(errorList && errorList.price), }"
            ...
            <BRow class="mb-3">
              ...
              <BCol md="10">
                <input class="form-control" id="form-description" placeholder="Masukkan Deskripsi Produk"
                  v-model="formModel.description" :class="{ 'is-invalid': !!(errorList && errorList.description) }" />

                <template v-if="!!(errorList && errorList.description)">
                  <div class="invalid-feedback" v-for="(err, index) in errorList.description" :key="index">
                    <span>{{ err }}</span>
                  </div>
                </template>
              </BCol>
                ...
                <BTr v-for="(detail, i) in details" :key="i">
                  ...
                  <BTh>
                    ...
                    <template v-if="!!(errorList && errorList[`details.${i}.description`])">
                      <div class="invalid-feedback" v-for="(err, index) in errorList[`details.${i}.description`]"
                        :key="index">
                        <span>{{ err }}</span>
                      </div>
                    </template>
                  ...
                  <BTh>
                    ...
                    <template v-if="!!(errorList && errorList[`details.${i}.type`])">
                      <div class="invalid-feedback" v-for="(err, index) in errorList[`details.${i}.type`]" :key="index">
                        <span>{{ err }}</span>
                      </div>
                    </template>
                  </BTh>
